...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText) {
		...
		String[] strArr = sourceText.split(" +");
		starter = strArr[0];
		...
		for (int i = 1; i <= strArr.length; i++) {
			...
			while (j < wordList.size()) {
				if (prevWord.equals(wordList.get(j).getWord())) {
					if (i == strArr.length) {
						wordList.get(j).addNextWord(strArr[0]);
						return;
					}
					wordList.get(j).addNextWord(strArr[i]);
					prevWord = strArr[i];
					flag = 1;
					break;
				}
			...
			if (flag == 0) {
				...
				if (i == strArr.length) {
					w.addNextWord(strArr[0]);
					wordList.add(w);
					break;
				} else {
					w.addNextWord(strArr[i]);
					wordList.add(w);
					prevWord = strArr[i];
				}
	...
	public String generateText(int numWords) {
		...
		while (i < numWords) {
			int j = 0;
			while (j < wordList.size()) {
				if (currWord.equals(wordList.get(j).getWord())) {
					String w = wordList.get(j).getRandomNextWord(new Random());
					output = output + " " + w;
					currWord = w;
					break;
				}
	...
	public static void main(String[] args) {
		...
		String textString2 = "You say yes, I say no, " + "You say stop, and I say go, go, go, "
				...
				+ "I don't know why you say goodbye, I say hello, hello, hello, "
				+ "I don't know why you say goodbye, I say hello, hello, hello,";
		// String textString2 ="Hello. Hello there. This is a test. Hello there.
...
class ListNode {
	...
	public String getRandomNextWord(Random generator) {
		...
		int randomNum = generator.nextInt(nextWords.size());// 0 inclusive, size
															// exclusive
